...
import Loading from '../../components/loading';
  const UpdateBook = () => {
    ...
    const { register, setValue,handleSubmit, formState: { errors },setError, watch } = useForm({
      mode:"onChange"
    });
    ...
    const onSubmit = async(data)=>{
      ...
      const oldPrcie = Number(data.oldPrice)
      const newPrice = Number(data.newPrice)
      
      if(isOffer && oldPrcie<=newPrice){
        setError("oldPrice",{
          type:"manual",message:"old price must be greater than new price or disable offer option"
        })
        return;
      }
      const finalOldPrice = isOffer?oldPrcie:0
      const finalNewPrice = newPrice
      ...
      navigate("/manage-books",{state:{updated:true}})
        ...
        Swal.fire({
          icon: "error",
        title: "Oops...",
        text: `Something went wrong  try again ! ${error.message}`,
      });
